<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=180&section=header&text=Rahul%20K&fontSize=42&fontColor=fff&animation=twinkling&fontAlignY=32&desc=Full+Stack+Developer+%7C+Building+for+the+web%2C+one+commit+at+a+time&descAlignY=62&descSize=16" width="100%"/>

<div align="center">

<a href="https://git.io/typing-svg"><img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=28&duration=3000&pause=1000&color=70A5FD&center=true&vCenter=true&width=750&lines=Hey+there%2C+I'm+Rahul+K+%F0%9F%91%8B;Full+Stack+Developer+from+Kerala+%F0%9F%87%AE%F0%9F%87%B3;React+%7C+Next.js+%7C+Node.js+%7C+Django;Building+AI-powered+tools+%26+web+apps" alt="Typing SVG" /></a>

</div>

<br/>

---

## 🧑‍💻 About Me

<img align="right" width="380" src="https://raw.githubusercontent.com/abhisheknaiidu/abhisheknaiidu/master/code.gif"/>

```javascript
const rahul = {
  name        : "Rahul K",
  location    : "Kerala, India 🌴",
  email       : "rahulvidx0696@gmail.com",

  stack       : ["React.js", "Next.js", "Node.js",
                 "Python", "Django", "PostgreSQL",
                 "MongoDB", "Tailwind CSS"],

  currentlyLearning: [
    "Advanced System Design",
    "Docker & DevOps Workflows",
    "AI/ML Integration in Web Apps",
  ],

  funFact     : "I once built a real-time gaming tournament platform — yes, it had live leaderboards 🎮",

  motto       : () => "Ship fast. Learn faster. Build things that matter.",
};
```

<br clear="right"/>

---

## 🛠️ Tech Stack

**Languages**

![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white)

**Frameworks & Libraries**

![React](https://img.shields.io/badge/React.js-61DAFB?style=flat-square&logo=react&logoColor=black)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat-square&logo=next.js&logoColor=white)
![Redux](https://img.shields.io/badge/Redux-764ABC?style=flat-square&logo=redux&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=node.js&logoColor=white)
![Express.js](https://img.shields.io/badge/Express.js-000000?style=flat-square&logo=express&logoColor=white)
![Django](https://img.shields.io/badge/Django-092E20?style=flat-square&logo=django&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-06B6D4?style=flat-square&logo=tailwind-css&logoColor=white)
![MUI](https://img.shields.io/badge/MUI-007FFF?style=flat-square&logo=mui&logoColor=white)

**Databases**

![PostgreSQL](https://img.shields.io/badge/PostgreSQL-336791?style=flat-square&logo=postgresql&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white)
![Supabase](https://img.shields.io/badge/Supabase-3ECF8E?style=flat-square&logo=supabase&logoColor=white)

**DevOps & Tools**

![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![Nginx](https://img.shields.io/badge/Nginx-009639?style=flat-square&logo=nginx&logoColor=white)
![VS Code](https://img.shields.io/badge/VS_Code-007ACC?style=flat-square&logo=visual-studio-code&logoColor=white)

---

## 📊 GitHub Stats

<div align="center">

<a href="https://github-readme-stats.vercel.app/api?username=rahulvidx&show_icons=true&theme=tokyonight&hide_border=true&include_all_commits=true&count_private=true&bg_color=0d1117&title_color=70a5fd&icon_color=bf91f3&text_color=c9d1d9&border_radius=10">
  <img height="180em" src="https://github-readme-stats.vercel.app/api?username=rahulvidx&show_icons=true&theme=tokyonight&hide_border=true&include_all_commits=true&count_private=true&bg_color=0d1117&title_color=70a5fd&icon_color=bf91f3&text_color=c9d1d9&border_radius=10"/>
</a>
<a href="https://github-readme-stats.vercel.app/api/top-langs/?username=rahulvidx&layout=compact&theme=tokyonight&hide_border=true&bg_color=0d1117&title_color=70a5fd&text_color=c9d1d9&langs_count=8&border_radius=10">
  <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=rahulvidx&layout=compact&theme=tokyonight&hide_border=true&bg_color=0d1117&title_color=70a5fd&text_color=c9d1d9&langs_count=8&border_radius=10"/>
</a>

</div>

---

## 🔥 Streak Stats

<div align="center">

<img src="https://streak-stats.demolab.com?user=rahulvidx&theme=tokyonight-duo&hide_border=true&background=0D1117&ring=70A5FD&fire=BF91F3&currStreakLabel=70A5FD&sideLabels=38BDAE&dates=8B949E&currStreakNum=C9D1D9&sideNums=C9D1D9&stroke=0D1117&border_radius=10" alt="GitHub Streak"/>

</div>

---

## 📈 Activity Graph

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=rahulvidx&theme=tokyo-night&bg_color=0d1117&color=70a5fd&line=bf91f3&point=38bdae&area=true&hide_border=true" width="100%"/>

</div>

---

## 🏆 Trophies

<div align="center">

<img src="https://github-profile-trophy.vercel.app/?username=rahulvidx&theme=tokyonight&no-frame=true&no-bg=true&row=1&column=7&margin-w=4"/>

</div>

---

## 💼 Work Experience

<details>
<summary><b>🏛️ IHRD Kerala — Software Developer</b> &nbsp;|&nbsp; Jun 2025 – Present &nbsp;|&nbsp; Kerala, India</summary>

<br/>

> ![React](https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black) ![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white) ![Django](https://img.shields.io/badge/Django-092E20?style=flat-square&logo=django&logoColor=white) ![PostgreSQL](https://img.shields.io/badge/PostgreSQL-336791?style=flat-square&logo=postgresql&logoColor=white)

- 🎬 Developing and maintaining the **Kerala Legislative Assembly Video Uploading Website** — a government-grade media platform
- 🔐 Implementing **secure data upload and management workflows** aligned with government security standards
- ⚡ Optimizing web performance and ensuring full compliance with government security protocols
- 🏗️ Working across the full stack: React frontend, Django REST backend, PostgreSQL data layer

</details>

<details>
<summary><b>🚀 Meta Scifor Technologies — Software Developer</b> &nbsp;|&nbsp; May 2024 – Jan 2025 &nbsp;|&nbsp; Remote</summary>

<br/>

> ![React](https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black) ![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat-square&logo=next.js&logoColor=white)

- 🤝 Collaborated with senior developers to integrate front-end and backend components for production web applications
- 🎨 Enhanced product **UI/UX using React.js and Next.js** through proactive issue resolution and code reviews
- ✅ Ensured seamless functionality and performance across integrated application modules
- 🔍 Actively participated in code reviews, raising code quality standards across the team

</details>

<details>
<summary><b>🧪 WebSoulLabs — Full Stack Developer Intern</b> &nbsp;|&nbsp; May 2023 – Nov 2023 &nbsp;|&nbsp; Remote</summary>

<br/>

> ![React](https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black) ![Redux](https://img.shields.io/badge/Redux-764ABC?style=flat-square&logo=redux&logoColor=white) ![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=node.js&logoColor=white) ![Tailwind CSS](https://img.shields.io/badge/Tailwind-06B6D4?style=flat-square&logo=tailwind-css&logoColor=white)

- 🎮 Built an **online gaming tournament platform** with account creation, real-time point tables, and live updates from scratch
- 🔒 Developed secure **authentication and membership systems** using React.js, Redux, Node.js, and Tailwind CSS
- 🛒 Created a fully responsive **e-commerce website** with online payments, add-to-cart, product Q&A, and user feedback features
- 📱 Delivered pixel-perfect, mobile-first responsive layouts across both platforms

</details>

---

## 🚀 Featured Projects

<div align="center">

| Project | Stack | Highlights |
|:--------|:------|:-----------|
| [**🤖 Ochha — AI Meeting Summarizer**](#) | Next.js (TS) · Node.js · AssemblyAI · Supabase · MongoDB | AI tool that captures & summarizes Google Meet conversations in real time; full-stack implementation across backend and frontend |
| [**🎮 Gaming Tournament Platform**](#) | React.js · Redux · Node.js · Tailwind CSS | Real-time point tables, account system, live tournament updates, and secure membership management |
| [**🛒 E-Commerce Platform**](#) | React.js · Node.js · Redux | Full-featured store with online payments, add-to-cart, product Q&A, and feedback system |

</div>

---

## 🎓 Education

<div align="center">

| Degree | Institution | Year |
|:-------|:------------|:-----|
| 🎓 B.Tech / MCA (Computer Science) | Kerala, India | 2023 |

</div>

---

## 📚 Currently Learning

```
🐳 Docker          → Multi-stage builds, Compose, container orchestration
⚙️  DevOps          → CI/CD pipelines, Nginx reverse proxy, PM2 process management
🤖 AI Integration  → LLM APIs, AssemblyAI, building AI-powered web tools
🧱 System Design   → Distributed systems, caching, scalable architectures
🔐 Security        → Auth flows, JWT, government-grade data handling standards
```

---

## 📬 Connect With Me

<div align="center">

[![Email](https://img.shields.io/badge/Email-rahulvidx0696%40gmail.com-EA4335?style=flat-square&logo=gmail&logoColor=white)](mailto:rahulvidx0696@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-rahulvidx-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/rahulvidx)
[![Phone](https://img.shields.io/badge/Phone-%2B91_8848652896-25D366?style=flat-square&logo=whatsapp&logoColor=white)](tel:+918848652896)

<img src="https://komarev.com/ghpvc/?username=rahulvidx&style=flat-square&color=70a5fd&label=Profile+Views" alt="Profile Views"/>

</div>

---

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=120&section=footer&animation=twinkling" width="100%"/>
